...
class DriverController extends Controller
{
    ...
    public function actionSetBuses($id) {
        $driver = $this->findModel($id);
     
        if(Yii::$app->request->isPost) {
            $buses = Yii::$app->request->post('buses');
            $driver->saveBuses($buses);
            return $this->redirect(['update', 'id' => $driver->id]);
//            var_dump($buses);die;
        }
        $selectedBuses = $driver->getSelectedBuses();
        $buses = ArrayHelper::map(Bus::find()->all(), 'id', 'model');
